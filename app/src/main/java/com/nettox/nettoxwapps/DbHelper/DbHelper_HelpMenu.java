...
public class DbHelper_HelpMenu extends SQLiteOpenHelper {
    ...
    public List<DbModel_HelpMenu> getListProduct () {
        DbModel_HelpMenu product = null;
        List<DbModel_HelpMenu> product_list = new ArrayList<>();
        openDatabase();

        Cursor cursor = dbHelper_sqLiteDatabase.rawQuery("SELECT * FROM " + TB_HELPMENU, null);
        cursor.moveToFirst();
